...
fn proxies_api_with_injected_token() {
    ...
    let target = RemoteTarget {
        base_url: format!("http://127.0.0.1:{rport}"),
        token: "SECRET".into(),
    };
    let local = spawn(ServerConfig {
        remote: Some(Arc::new(FixedRemote(target))),
        ..base(0)
    })
    .expect("local");
    ...
    let get = ureq::get(&format!("http://127.0.0.1:{lport}/api/secrets-status"))
        .call()
        .expect("proxied GET");
    assert_eq!(
        get.status(),
        200,
        "GET /api/secrets-status should proxy to the remote"
    );
    ...
    assert_eq!(
        post.status(),
        200,
        "POST /api/git-dirty-many should proxy to the remote"
    );

    // The connection manager itself is handled LOCALLY (never proxied) → 200.
    let status = ureq::get(&format!("http://127.0.0.1:{lport}/api/remote/status"))
        .call()
        .expect("local remote/status");
    assert_eq!(
        status.status(),
        200,
        "/api/remote/* must be served locally even while connected"
    );
    ...
    assert!(
        matches!(direct, Err(ureq::Error::Status(401, _))),
        "remote must reject an un-tokened request"
    );
